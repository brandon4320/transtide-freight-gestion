...
const FRANCO = {
  ...
  fleteIntl:  [],
  proveedores:[
    { id: 1, nombre: 'karting',     m3: 20.34, fobUSD: 25314, tributosUSD: 3991.87, tributosTC: 1390 },
    { id: 2, nombre: 'gimnasio',    m3: 13,    fobUSD: 5500,  tributosUSD: 2746.04, tributosTC: 1390 },
    { id: 3, nombre: 'generadores', m3: 4.8,   fobUSD: 16000, tributosUSD: 1176.05, tributosTC: 1390 },
  ],
  cobrar:[
...
function OperationDetail({ op, onBack }) {
  const [mainTab,   setMainTab]   = useState('gastos');
  ...
  const [proveedores, setProveedores] = useState([...FRANCO.proveedores, { id: 999, nombre: '', m3: '', fobUSD: '', tributosUSD: '', tributosTC: '' }]);
  ...
  const calc = useMemo(() => {
    ...
    const perProv = proveedores
      ...
      .map((p, i) => {
        ...
        const fobPlus     = n(p.fobUSD) + gastosUSD;
        const honorarios  = cb.honorarios ? Math.round(fobPlus * 0.04 * 100) / 100 : 0;
        const totalUSD    = Math.round((gastosUSD + honorarios + n(cb.despAdic)) * 100) / 100;
        return { nombre: p.nombre, m3: n(p.m3), fobUSD: n(p.fobUSD), ratio, prorPesos, tributoPesos, costoFinal, gastosUSD, fobPlus, honorarios, totalUSD, cb };
      });
  ...
  return (
    <div style={{ paddingBottom: '3rem' }}>
      ...
      <div style={{ display: 'flex', background: '#fff', borderRadius: '12px', padding: '4px', border: '1px solid #e2e8f0', gap: '3px', marginBottom: '1.25rem', width: 'fit-content' }}>
        {[['gastos','Gastos'],['proveedores','Proveedores & VEP'],['acobrar','A Cobrar (USD)']].map(([id, lbl]) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={CARD}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
              ...
              <p style={{ fontSize: '0.85rem', fontWeight: 700, color: '#1e293b' }}>Proveedores — m³ y FOB</p>
            </div>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#fff7ed' }}>
                    {['Proveedor', 'm³ (CBM)', 'FOB USD', '% Ocupación'].map((h, i) => (
                      <th key={h} style={{ ...TH, color: '#92400e', background: 'none', width: i === 0 ? '35%' : '21%' }}>{h}</th>
                    ))}
                    <th style={{ width: '7%' }}></th>
                  </tr>
                </thead>
                <tbody>
                  {proveedores.map((p, i) => {
                    ...
                    const isTot   = i >= proveedores.length - 1 && p.nombre === '';
                    return (
                      <tr key={p.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafbfc' }}>
                        ...
                        <td style={TD}>
                          {p.nombre && <button onClick={() => setProveedores(pr => pr.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5e1' }}>×</button>}
                        </td>
                      </tr>
                    );
                  })}
            ...
            <button onClick={() => { setProveedores(p => [...p, { id: Date.now(), nombre: '', m3: '', fobUSD: '', tributosUSD: '', tributosTC: '' }]); setCobrar(c => [...c, { tc: '', honorarios: true, despAdic: '' }]); }}
      ...
      {mainTab === 'acobrar' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  ...
                  <tr style={{ background: '#f8fafc' }}>
                    {['Proveedor','m³','T.C. conv.','Gastos $ → USD','FOB USD','FOB + Gastos','Honor. 4%','Desp. Adic. USD','Activo',''].map(h => (
                ...
                <tfoot>
                  <tr style={{ background: '#1e293b' }}>
                    <td colSpan={3} style={{ ...TD, color: '#fff', fontWeight: 700, borderBottom: 'none' }}>TOTAL</td>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
            {calc.perProv.map(p => (
              <div key={p.nombre} style={{ ...CARD, borderTop: `3px solid #7c3aed` }}>
                <p style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.5rem' }}>{p.nombre}</p>
                <p style={{ fontSize: '1.5rem', fontWeight: 800, color: '#7c3aed', lineHeight: 1, marginBottom: '0.4rem' }}>{fmtU(p.totalUSD)}</p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.4rem', marginTop: '0.75rem', paddingTop: '0.75rem', borderTop: '1px solid #f1f5f9' }}>
                  {[['Gastos', fmtU(p.gastosUSD)], ['FOB', fmtU(p.fobUSD)], ['Honor.', fmtU(p.honorarios)], ['Desp.', fmtU(n(cobrar[calc.perProv.indexOf(p)]?.despAdic))]].map(([l,v]) => (
