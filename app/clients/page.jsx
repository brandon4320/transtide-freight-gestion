...
const INP  = { width: '100%', padding: '0.5rem 0.75rem', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '8px', fontSize: '0.85rem', color: '#e8edf5', background: '#0b1220', outline: 'none', boxSizing: 'border-box' };
const LBL  = { display: 'block', fontSize: '0.65rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.3rem' };
  ...
  return (
    <div style={{ paddingBottom: '3rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#e8edf5', marginBottom: '0.2rem' }}>Clientes</h2>
          <p style={{ fontSize: '0.82rem', color: '#475569' }}>{clientes.length} clientes registrados</p>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#0b1220', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '0.45rem 0.75rem' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar cliente..." style={{ border: 'none', outline: 'none', fontSize: '0.82rem', color: '#e8edf5', width: '180px', background: 'transparent' }} />
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        {[
          ['Clientes totales', clientes.length, '#ea580c', 'rgba(234,88,12,0.12)'],
          ['Con CUIT', clientes.filter(c=>c.cuit).length, '#a78bfa', 'rgba(167,139,250,0.12)'],
          ['Con email', clientes.filter(c=>c.email).length, '#10b981', 'rgba(16,185,129,0.12)'],
        ].map(([lbl, val, color, bg]) => (
          <div key={lbl} style={{ ...CARD, background: bg, border: `1px solid ${color}30` }}>
            <p style={{ fontSize: '0.68rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.4rem' }}>{lbl}</p>
      ...
      {filtered.length === 0 ? (
        <div style={{ ...CARD, textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#1e293b" strokeWidth="1.5" style={{ marginBottom: '0.75rem' }}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
          <p style={{ fontWeight: 600, marginBottom: '0.3rem', color: '#94a3b8' }}>{search ? 'Sin resultados' : 'No hay clientes aún'}</p>
          <p style={{ fontSize: '0.8rem' }}>{search ? 'Probá con otro término' : 'Agregá tu primer cliente'}</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
          {filtered.map(c => (
            <div key={c.id} style={{ ...CARD, borderTop: '3px solid #ea580c', position: 'relative' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: 'rgba(234,88,12,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', fontWeight: 800, color: '#ea580c' }}>
                  {c.nombre.charAt(0).toUpperCase()}
                </div>
                <div style={{ display: 'flex', gap: '0.4rem' }}>
                  <button onClick={() => openEdit(c)} style={{ padding: '0.3rem 0.6rem', borderRadius: '7px', border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(255,255,255,0.04)', color: '#94a3b8', fontSize: '0.72rem', fontWeight: 600, cursor: 'pointer' }}>Editar</button>
                  <button onClick={() => setConfirm(c.id)} style={{ padding: '0.3rem 0.6rem', borderRadius: '7px', border: '1px solid rgba(239,68,68,0.25)', background: 'rgba(239,68,68,0.06)', color: '#ef4444', fontSize: '0.72rem', fontWeight: 600, cursor: 'pointer' }}>×</button>
                </div>
              </div>
              <p style={{ fontWeight: 700, color: '#e8edf5', fontSize: '0.95rem', marginBottom: '0.15rem' }}>{c.nombre}</p>
              {c.cuit && <p style={{ fontSize: '0.75rem', color: '#475569', marginBottom: '0.5rem' }}>CUIT {c.cuit}</p>}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginTop: '0.75rem', paddingTop: '0.75rem', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                {c.email && (
                ...
                {c.notas && <p style={{ fontSize: '0.72rem', color: '#475569', marginTop: '0.25rem', fontStyle: 'italic' }}>{c.notas}</p>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setModal(null)}>
          <div style={{ ...CARD, width: '100%', maxWidth: '480px', margin: '1rem', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <h3 style={{ fontSize: '1rem', fontWeight: 800, color: '#e8edf5' }}>{modal === 'new' ? 'Nuevo cliente' : 'Editar cliente'}</h3>
              <button onClick={() => setModal(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#475569', fontSize: '1.2rem' }}>×</button>
            </div>
            <div style={{ display: 'grid', gap: '0.85rem' }}>
              ...
              <div>
                ...
                <textarea value={form.notas} onChange={e => setForm(f => ({ ...f, notas: e.target.value }))} rows={2} style={{ ...INP, resize: 'vertical', fontFamily: 'inherit', colorScheme: 'dark' }} placeholder="Observaciones opcionales..." />
              </div>
            </div>
      ...
      {confirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setConfirm(null)}>
          <div style={{ ...CARD, width: '100%', maxWidth: '360px', margin: '1rem', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontWeight: 700, color: '#e8edf5', marginBottom: '0.4rem' }}>¿Eliminar cliente?</p>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
              ...
              <button onClick={() => remove(confirm)} style={{ padding: '0.5rem 1.2rem', borderRadius: '8px', border: 'none', background: '#ef4444', color: '#fff', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer' }}>Eliminar</button>
            </div>
          </div>
        </div>
      )}
